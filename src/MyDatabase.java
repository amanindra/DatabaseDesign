...
public class MyDatabase {
	...
	public static void main(String[] args) {
		...
		try {
			dbMasterTableFile = new RandomAccessFile(Utility.getFilePath(
					"master", masterTableName), "rw");
		...
		while (!isExit) {
			...
			if (userCommand.length() < 5
					...
					&& (Integer.valueOf(userCommand.substring(1)) > 0 || Integer
							.valueOf(userCommand.substring(1)) < 0)) {
